...
- **Bug/comentario incorrecto:** `app/login/page.tsx` dice que vuelve "a la home (o página previa)" pero siempre hace `router.replace("/")` — confirmado navegando en vivo desde `/contacto`, termina en `/` igual. Detalle en [`cambios/2026-09-10-bugs-resto-proyecto.md`](./cambios/2026-09-10-bugs-resto-proyecto.md).
- **Sin feedback:** boletín del footer (`Footer.tsx`) no muestra mensaje de éxito/error al enviar; favoritos/comparar en `FeaturedOffers.tsx` no cambian de estado al hacer clic (solo `stopPropagation`).
- **Detalle de diseño:** `WrenchCursor.tsx` trata los campos de texto (`input`/`textarea`) igual que botones/links (mismo ícono "interactivo"), perdiendo la afordancia del cursor de texto nativo (que está oculto globalmente).

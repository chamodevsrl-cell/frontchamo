...
> Escrita en `main` antes de que existiera esta rama de features. Los 2 hallazgos
> reales (ProductModal sin scroll-to-top, gap fijo en `CategoriesGrid`) se revisaron
> contra el código de esta rama al mergear (2026-09-11): el del modal ya no aplicaba
> (se resuelve solo vía `key={selected.id}`) y el de `CategoriesGrid` se corrigió — ver
> [`2026-09-11-merge-ready-fixes.md`](./2026-09-11-merge-ready-fixes.md). Se renombró
> de `2026-09-10-bugs-cambios-recientes.md` a este nombre porque esta rama ya tenía un
> archivo distinto con ese mismo nombre (intro/WhatsApp/favoritos).
